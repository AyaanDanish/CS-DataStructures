...
public:
    ...
    void pushNode(int value) {
        ...
        newNode->next = head;            //Next part of new node now points to former head (former first node)
        head->prev = newNode;            //Prev of the former first node now points to the new first node
    ...
    void insertNodeAt(int value, int location) {
        //If the user wanted to add a new node at location 1 (the start), let the pushNode function handle it instead
        if (location == 1) {
            pushNode(value);
            return;
        }
        //If the user wanted to add a new node at the end of the LL, let the addNodeToEnd function handle it
        if (location == countNodes() + 1) {
            addNodeToEnd(value);
            return;
        }

        //Everything in between, this function will handle
        Node *newNode = new Node(value); //Create new node and store the value provided
        ...
        ptr->next->prev = newNode; //Make the reverse connection of the one made above
        ptr->next = newNode;       //Then set the next of the current node to point to the new one (attach from the left)
        newNode->prev = ptr;       //Make the reverse connection of the one made above
    }
    ...
    void deleteNode(int location) {
        ...
        //If deleting last node, handle that specially
        if (location == countNodes()) {
            while (ptr->next != NULL) { //Traverse to the last node, keeping the prev ptr on second last node
                prev = ptr;
                ptr = ptr->next;
            }
            prev->next = NULL; //Second last node now points to NULL
            delete ptr;        //Last node gets deleted
            return;
        }
